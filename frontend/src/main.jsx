...
function Login({ onConnected }) {
  ...
  const openLogin = async () => {
    try { const result = await api('/api/auth/login-url'); window.open(result.url, '_blank', 'noopener,noreferrer') }
...
function SignalsTab() {
  const [params, setParams] = useState({ short_sma: 6, long_sma: 30, lookback_days: 365, max_stocks: 20 }); const [results, setResults] = useState([]); const [meta, setMeta] = useState(null); const [error, setError] = useState(''); const [busy, setBusy] = useState(false)
  const update = (key) => (event) => setParams({ ...params, [key]: Number(event.target.value) })
  const generate = async () => { setBusy(true); setError(''); try { const data = await api('/api/signals', { method: 'POST', body: JSON.stringify(params) }); setResults(data.results); setMeta(data) } catch (err) { setError(err.message) } finally { setBusy(false) } }
...
function App() { const [connected, setConnected] = useState(false); useEffect(() => { api('/api/auth/status').then((data) => setConnected(data.connected)).catch(() => {}) }, []); return connected ? <Dashboard onLogout={() => setConnected(false)} /> : <Login onConnected={() => setConnected(true)} /> }
